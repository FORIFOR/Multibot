...
## 9Bの実行から見つかった入力とレビューの問題

最初の9B実行は、メモで確認できない「前回と同じ納期」「添付ファイル」や日本語の誤記を含むメールを生成しました。Reviewerもこれを見逃しています。自動採点の通過はこの意味上の正確性を保証しません。

調査すると、計画担当は添付の名前と文字数しか受け取っておらず、本文を参照できませんでした。計画に添付本文を渡し（1添付12000文字まで、切り詰めを明記）、元の入力から受入条件を作るよう修正しました。制作・レビュー担当にも、入力本文と公開済みartifactの違いを明記しています。

また、同じ受入条件IDを2回提出したレビューが受理されていました。[実際の入力・計画・レビュー](qwen35-review-input.json)を回帰テストに使い、重複を拒否し、提出済みにしないよう修正。回帰テストは73件passです。

## 完了状態の公開順序

CIで終了ログの取得件数が103→104と変わる競合を検出しました。最終レポートを保存する前にDBの状態をcompletedにしていたため、UIや取得タイミング次第で未完成の出力が見えました。レポートと終了イベントを保存してから状態を更新し、その後にSSE通知するよう変更しました。

[実際のsingle runのメタデータ](single-finalization.json)と同じsingle設定で、モデル呼び出しを使わない実際のレポート保存処理を再生。完了状態が見えた時点で、レポート・終了イベントが揃うことを確認しています。実行結果の品質を再認定するテストではありません。回帰テストは74件passです。

## 宛先・目的の保持

本文を渡した後も、9Bの非推論設定で「A社向けメール」を社内の山田宛にする取り違えを確認しました。計画・制作・レビューの各指示で、元の依頼の宛先・対象読者・目的を保持し、参照メモの全項目を追加要件にしないよう明記しています。

推論を有効にした9Bの追加試験も別設定で行います。Ollamaの接続probeだけは推論を無効化して短いtool/JSON出力を確認し、業務実行は接続で指定した推論設定を使います。接続probeは推論品質の検証ではありません。

最終レポート作成中の停止も検証し、停止要求後にcompletedを公開せず、モデルを再呼出ししないcancelledレポートを残すよう補いました。回帰テスト75件pass。

## 時間切れの理由と終了イベント

推論有効の実試験では600秒で正しく中断しましたが、DBのblocked_reasonがnull、run.interruptedが具体的理由と汎用理由で2回記録されていました。[実行記録](thinking-interruption.json)の消費時間を再生し、Schedulerから終了処理へ理由を渡し、イベントを1回だけ保存してから状態更新・SSE通知するよう修正しました。実モデルを模した応答は使っていません。新規11件を含む全76件の回帰テストが通過しました。
