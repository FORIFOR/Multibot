...
50課題×3回×2方式の300件は、修正版を固定した別系列として実行します。時間上限600秒、名目予算$6、同じモデル・課題・設定を固定し、失敗と途中成果物も保持します。ローカル推論API料金は$0ですが、電力・端末代は計測していません。基本採点の合格は原文との一致や業務品質の合格とは区別します。旧系列・推論有効系列とは集計を混ぜません。自動フォローは有効で、正常進行中の定例通知は行いません。
