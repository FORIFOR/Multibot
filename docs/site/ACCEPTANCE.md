...
| 軸 | Agent Team（公開ページ、改稿直後） | VoiceOS |
| --- | --- | --- |
| Performance | 97（日）/ 100（英） | 77 |
| Accessibility | 96 → 改修後のローカル測定で 100 | 100 |
| Best Practices | 100 | 79 |
| SEO | 100 | 100 |
| LCP | 1.1秒 / 0.9秒 | 5.2秒 |
| 転送量・リクエスト数 | 18KB・5（改修後は画面の画像を加えて日169KB・英91KB） | 748KB・77 |
| JavaScriptなしで読める | 全セクション（`site-check.mjs` で確認） | 未確認 |
| 動きを止める手段・reduced-motion | あり（確認済み） | 未確認 |

Accessibility で負けていた原因はフッターの文字色のコントラスト比 4.46（基準 4.5）。補足色を `#586079` に変え、あわせて `figure` への `tabpanel` と、操作ボタンを含む要素への `role="img"` を外した。改修後のローカル測定は日英とも 4 項目 100、不合格の監査 0 件。

VoiceOS にあって、こちらに無いもの: 推薦文10件、利用者数の表示、Y Combinator の表記。実在する根拠がないため載せない（[DESIGN.md](DESIGN.md)）。代わりに、実際のアプリ画面3枚、記録原文つきの指摘3件、不利な結果を含む比較試験と証跡リンクを置いた。

**「超えた」と言える範囲**: 上の測定できる軸（速度、軽さ、ベストプラクティス、JavaScriptなしでの可読性）と、主張ごとに証跡へたどれること。**言えない範囲**: 見た人が製品を理解して使い始める割合。利用者による評価も、公開後の実測（CWVのフィールド値、CTAのクリック）もまだ無い。
