...
The [v11 qwen3.5 series](evidence/real-readiness-v11-qwen35-2026-09-14/README.md) corrected that plan error. Runs 1 and 2 completed with a real local model after rejected revisions, delivery Schema pass and independent Reviewer pass; run 3 stopped partial after repeated no-tool output and an unauthorized exception action. Two accepted runs are not the ten-run L1 condition, and L2/L3 remain open.

The [v12 qwen3.5 series](evidence/real-readiness-v12-qwen35-2026-09-14/README.md) exercised Runtime context compaction after a real `max_tokens` failure. Run 1 completed under the previous contract after a rejected revision, delivery Schema pass and independent Reviewer pass. A follow-up contract test now rejects three half-transliterated terms that this artifact still contained (`キーcloak`, `パインされた`, `バックス`); the earlier review result is preserved and is not rewritten. Under the current contract, v12 run 1 is not accepted. Run 2 failed its delivery contract and was interrupted at the 1200-second wall-clock limit before Reviewer, while run 3 was interrupted when the series was stopped. The historical count is 3/20, while the current-contract accepted count is 2/20, so the ten-run L1 condition and L2/L3 remain open.
