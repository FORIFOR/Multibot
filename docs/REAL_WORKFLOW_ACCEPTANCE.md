...
The [v12 qwen3.5 series](evidence/real-readiness-v12-qwen35-2026-09-14/README.md) exercised Runtime context compaction after a real `max_tokens` failure. Run 1 completed after a rejected revision, delivery Schema pass and independent Reviewer pass. A follow-up contract test now rejects three half-transliterated terms that this accepted artifact still contained (`キーcloak`, `パインされた`, `バックス`); the earlier review result is preserved and is not rewritten. Run 2 failed its delivery contract and was interrupted at the 1200-second wall-clock limit before Reviewer, while run 3 was interrupted when the series was stopped. The combined accepted count is 3/20, so the ten-run L1 condition and L2/L3 remain open.
