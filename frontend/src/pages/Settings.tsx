...
export default function Settings() {
  ...
  const guard = async (fn: () => Promise<unknown>, ok: string) => {
    ...
    try { await fn(); setNote(ok); await load() } catch (e) {
      if (e instanceof ApiError && e.status === 409) { setErr('設定が別の場所で更新されました。再読み込みします。'); await load() } else setErr(String(e))
    }
  }
  if (!cfg) return <p className="muted">{err || '読み込み中…'}</p>
  return (
    <div className="stack">
      <div className="row">
        <h1>設定</h1>
        <span className="tag">revision {cfg.revision}</span>
        <span className="muted small">変更は次の run から反映されます。進行中の run は開始時のスナップショットで動きます。</span>
      </div>
      ...
      {cfg.problems.length === 0 && <div className="banner ok">開始条件を満たしています。</div>}
      {err && <p className="err">{err}</p>}
      {note && <p className="small" style={{ color: 'var(--ok)' }}>{note}</p>}
      <div className="settings">
        <div className="stack">
          <h3>接続（API キーは参照のみ保存: env:NAME / keychain:service/account / file:/path）</h3>
...
function ConnectionCard({ c, cfg, guard }: { c: Connection; cfg: Config; guard: (fn: () => Promise<unknown>, ok: string) => Promise<void> }) {
  ...
  return (
    <div className="card stack">
      ...
      <div className="row">
        <input className="input" style={{ flex: 1 }} value={keyRef} onChange={(e) => setKeyRef(e.target.value)} placeholder="api_key_ref 例: env:ANTHROPIC_API_KEY" />
        {driver === 'anthropic_messages' && <label className="lock" title="Anthropic のサーバ側 refusal fallback（別モデルへ自動ルーティング）。既定 OFF。"><input type="checkbox" checked={fallback} onChange={(e) => setFallback(e.target.checked)} /> refusal fallback</label>}
        <button className="btn sm" disabled={!dirty} onClick={() => guard(() => api.putConnection(c.id, { expected_revision: cfg.revision, driver, base_url: baseUrl, api_key_ref: keyRef || null, refusal_fallback: fallback }), '接続を保存しました（要 再疎通確認）')}>保存</button>
      </div>
      <div className="row">
        <input className="input" style={{ width: 240 }} value={probeModel} onChange={(e) => setProbeModel(e.target.value)} placeholder="probe model" />
        <button className="btn sm ghost" disabled={probing || dirty} onClick={async () => { setProbing(true); await guard(() => api.probe(c.id, probeModel), '疎通確認を実行しました'); setProbing(false) }}>{probing ? '確認中…' : '疎通確認（実 API 呼出・少額）'}</button>
        <span className="muted small">tool calling と JSON schema 出力を実際に試します。</span>
      </div>
...
function NewConnection({ cfg, guard }: { cfg: Config; guard: (fn: () => Promise<unknown>, ok: string) => Promise<void> }) {
  ...
  return (
    <details className="card flat">
      <summary className="muted small">接続を追加（例: ローカル Ollama = driver ollama, http://localhost:11434/v1, キーなし）</summary>
      <div className="row" style={{ marginTop: 8 }}>
        ...
        <button className="btn sm" disabled={!id || !baseUrl} onClick={() => guard(() => api.putConnection(id, { expected_revision: cfg.revision, driver, base_url: baseUrl, api_key_ref: keyRef || null }), '接続を追加しました')}>追加</button>
      </div>
    </details>
  )
}
...
function LimitsCard({ cfg, guard }: { cfg: Config; guard: (fn: () => Promise<unknown>, ok: string) => Promise<void> }) {
  ...
  return (
    <div className="card stack">
      <div className="row">
        <label className="small muted">既定モデル</label><input className="input" style={{ width: 220 }} value={model} onChange={(e) => setModel(e.target.value)} />
        <label className="small muted">既定接続</label>
      ...
      <details><summary className="muted small">価格表（USD / 1M tokens）。価格不明のクラウドモデルは開始できません。</summary>
        <textarea className="input mono" style={{ minHeight: 120 }} value={pricing} onChange={(e) => setPricing(e.target.value)} /></details>
      <div><button className="btn sm" onClick={() => guard(() => api.putLimits({ expected_revision: cfg.revision, limits, defaults: { model, connection_id: conn }, pricing: JSON.parse(pricing || '{}') }), '既定と上限を保存しました')}>保存</button></div>
    </div>
  )
}
  ...
  return (
    <div className="agentcard">
      <header>
        ...
        <label className="lock" style={{ marginLeft: 'auto' }} onClick={() => patch({ enabled: !a.enabled }, `${a.id} を${a.enabled ? '無効' : '有効'}化しました`)}><span className={'switch' + (a.enabled ? ' on' : '')} /> {a.enabled ? 'enabled' : 'disabled'}</label>
      </header>
      <div className="grid">
        <div><label>connection</label>
          <select className="input" value={a.connection_id} onChange={(e) => patch({ connection_id: e.target.value }, '接続を変更しました')}>
        ...
        <div><label>model</label><input className="input" defaultValue={a.model} onBlur={(e) => { if (e.target.value !== a.model) patch({ model: e.target.value || 'inherit' }, 'モデルを変更しました') }} placeholder={`inherit (${cfg.defaults.model})`} /></div>
        <div><label>effort</label>
          <select className="input" value={a.effort || ''} onChange={(e) => patch({ effort: e.target.value }, 'effort を変更しました')}>{EFFORTS.map((x) => <option key={x} value={x}>{x || 'default'}</option>)}</select></div>
        <div><label>実効</label><div className="mono small">{eff ? `${eff.model} @ ${eff.connection_id}/${eff.driver}` : '—'}</div></div>
      </div>
      <div className="row" style={{ marginTop: 8 }}>
        <span className="muted small">skills: {a.skill_ids.join(', ') || '—'}</span>
        <span className="muted small">tools: {a.tools.length}</span>
        <button className="btn sm ghost" style={{ marginLeft: 'auto' }} onClick={() => setOpen(!open)}>{open ? 'プロンプトを閉じる' : 'システムプロンプト'}</button>
      </div>
      {open && (
        <div className="stack" style={{ marginTop: 8 }}>
          <div className="row">
            <label className="lock" onClick={() => patch({ prompt_mode: a.prompt_mode === 'user_locked' ? 'auto_seed' : 'user_locked' }, 'ロック状態を変更しました')}>
          ...
          <div className="row">
            <button className="btn sm" disabled={!promptDirty} onClick={() => patch({ system_prompt_override: prompt }, 'プロンプトを保存しました（新 revision）')}>保存</button>
            <button className="btn sm ghost" disabled={a.system_prompt_override == null} onClick={() => patch({ reset_prompt: true }, '同梱プロンプトに戻しました')}>元に戻す</button>
