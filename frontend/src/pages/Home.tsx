    ...
    <div>
      <section className="hero">
        <div className="reveal">
          <h1>依頼は一度。<br />チームが作り、<em>確かめ</em>、経緯を残す。</h1>
          <p className="lede">Master が成果物と完了条件を決め、必要な Bot だけが実際に作業します。成果物・Bot 間の実メッセージ・時系列は同じ実行記録から表示されます。台本の会話や固定の成功ログは使いません。</p>
          {cfg && (
            <div className={'banner ' + (ready ? 'ok' : 'warn')} style={{ marginTop: 18 }}>
              {ready ? <>接続 <code>{cfg.defaults.connection_id}</code> / モデル <code>{model}</code> は疎通確認済み。予算上限 {money(cfg.limits.budget_usd)} / run。</>
                : <>開始前に設定が必要です：{cfg.problems.map((p) => p.message).join(' / ')} → <Link to="/settings" nav={nav}>設定を開く</Link></>}
            </div>
          )}
        </div>
        <div className="ask reveal">
          <textarea className="input" placeholder="例: この製品の説明をもとに、紹介LPとREADME、SNS投稿案を作って。足りない情報は調べて、公開前の状態まで仕上げて。" value={goal} onChange={(e) => setGoal(e.target.value)} />
          <details className="more">
            <summary>添付・URL・予算</summary>
            <div className="stack" style={{ marginTop: 8 }}>
              <textarea className="input" placeholder="製品説明などのテキスト（任意）" value={text} onChange={(e) => setText(e.target.value)} />
              <input className="input" placeholder="参照URL（空白区切り、任意）" value={urls} onChange={(e) => setUrls(e.target.value)} />
          ...
          <div className="foot">
            <span className="muted small">外部への投稿・送信は行いません（草案まで）。</span>
            <span className="spacer" />
            <button className="btn signal" disabled={!goal.trim() || busy || !ready} onClick={start}>{busy ? '開始中…' : '開始'}</button>
          </div>
          {err && <p className="err">{err}</p>}
          {problems.length > 0 && <div className="banner">{problems.map((p) => <div key={p.code}>{p.message}</div>)} <Link to="/settings" nav={nav}>設定へ</Link></div>}
        </div>
      </section>
      <section className="runs-list">
        <h3>最近の実行</h3>
        {runs.length === 0 && <p className="muted">まだ実行はありません。</p>}
        {runs.map((r) => (
