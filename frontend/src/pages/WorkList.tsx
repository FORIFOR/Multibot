...
const filters = ['all', 'active', 'attention', 'stopped', 'completed'] as const
...
function matches(run: Run, filter: Filter) {
  ...
  // Unfinished outcomes get their own view instead of being findable only under "all".
  if (filter === 'stopped') return ['partial', 'failed', 'interrupted', 'cancelled'].includes(run.status)
