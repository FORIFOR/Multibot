  ...
  return (
    <div className="shell">
      ...
      <main className={'main' + (runMatch ? ' wide' : '')}>
        {runMatch ? <RunView runId={runMatch[1]} nav={nav} /> : path.startsWith('/settings') && identity.role === 'admin' ? <Settings /> : <Home nav={nav} readOnly={identity.role === 'viewer'} canConfigure={identity.role === 'admin'} />}
      </main>
    </div>
  )
}
