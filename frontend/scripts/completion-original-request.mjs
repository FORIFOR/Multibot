...
try {
 ...
 await page.locator('#run-attachments').setInputFiles(resolve(process.env.QUALITY_SOURCE || '../docs/quality/integration.md'))
 ...
 const initial=await(await context.request.get(`http://127.0.0.1:8801/api/runs/${id}`)).json()
