...
try {
  for (const [width, lang] of [[1440,'ja'],[390,'ja'],[320,'ja'],[768,'en']]) {
    ...
    await noOverflow(page)
    await page.goto(base+'/runs/ui-fixture');await page.locator('.bot-agent').first().waitFor()
    const states=await page.locator('.bot-agent [data-bot-state]').evaluateAll(xs=>xs.map(e=>e.getAttribute('data-bot-state')))
    assert.deepEqual(states,['idle','researching','waiting','waiting','idle'])
    assert.equal(await page.locator('.chat-card [data-bot-state]').getAttribute('data-bot-state'),'idle')
