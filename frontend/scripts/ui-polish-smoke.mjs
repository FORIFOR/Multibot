...
try {
  for (const [width, lang] of [[1440,'ja'],[390,'ja'],[320,'ja'],[768,'en']]) {
    ...
    await form.locator('.bot-emoji-input[aria-invalid="false"]').waitFor()
    const pick = form.getByRole('button', {name:lang==='ja'?'パンダ':'Panda',exact:true})
    await pick.focus(); await page.keyboard.press('Space')
    await form.locator('.emoji-choices button[aria-pressed="true"]', {hasText:'🐼'}).or(form.getByRole('button', {name:lang==='ja'?'パンダ':'Panda',exact:true, pressed:true})).first().waitFor({timeout:5000})
    ...
    await noOverflow(page)
    await page.goto(base+'/runs/ui-fixture');await page.locator('.desk-teammate').first().waitFor()
    const states=await page.locator('.desk-teammate [data-bot-state]').evaluateAll(xs=>xs.map(e=>e.getAttribute('data-bot-state')))
    assert.deepEqual(states,['idle','researching','waiting','waiting','idle'])
    assert.equal(await page.locator('.conversation-message [data-bot-state]').getAttribute('data-bot-state'),'idle')
