...
def test_observed_v12_half_transliterations_are_rejected_by_the_current_contract():
    """The first v12 accepted report still contained three garbled terms."""
    script = Path(__file__).resolve().parents[1] / 'scripts' / 'production_workflow.py'
    module_spec = importlib.util.spec_from_file_location('v12_translation_contract_workflow', script)
    workflow = importlib.util.module_from_spec(module_spec); module_spec.loader.exec_module(workflow)
    expected = workflow.source_rows((EVIDENCE.parents[1] / 'PRODUCTION_PLAN.md').read_text())
    raw = (V12 / '01-run_1a09e7a5cdc83e9f62a' / 'artifact-002.bin').read_bytes()
    result = workflow.json_schema_check(raw, workflow.delivery_schema(expected))
    assert result['status'] == 'fail'
    assert any(term in problem for problem in result['problems'] for term in ('キーcloak', 'パインされた', 'バックス'))


def test_observed_v18_translation_drift_is_rejected_by_the_current_contract():
    """The stopped real-source series exposed four additional drift fragments."""
    script = Path(__file__).resolve().parents[1] / 'scripts' / 'production_workflow.py'
    module_spec = importlib.util.spec_from_file_location('v18_translation_contract_workflow', script)
    workflow = importlib.util.module_from_spec(module_spec); module_spec.loader.exec_module(workflow)
    expected = workflow.source_rows((EVIDENCE.parents[1] / 'PRODUCTION_PLAN.md').read_text())
    raw = (V18 / '02' / 'artifact-002.bin').read_bytes()
    result = workflow.json_schema_check(raw, workflow.delivery_schema(expected))
    assert result['status'] == 'fail'
...
def test_observed_v23_mixed_translation_drift_is_rejected_by_the_current_contract():
    """v23 mechanical passes still contained mixed or misspelled technical terms."""
    script = Path(__file__).resolve().parents[1] / 'scripts' / 'production_workflow.py'
    module_spec = importlib.util.spec_from_file_location('v23_translation_contract_workflow', script)
    workflow = importlib.util.module_from_spec(module_spec); module_spec.loader.exec_module(workflow)
    expected = workflow.source_rows((EVIDENCE.parents[1] / 'PRODUCTION_PLAN.md').read_text())
    observed = {
        'observed-rep02-readiness.json': ('ハードening', 'アクター監査'),
        'observed-rep07-readiness.json': ('デシフryption', 'キールドリル'),
        'observed-rep10-readiness.json': ('シネティック', 'エGRESS', '承約'),
    }
...
def test_observed_v25_revocation_translation_drift_is_rejected_by_the_current_contract():
    """v25 exposed a false-looking Japanese synonym for token revocation."""
    script = Path(__file__).resolve().parents[1] / 'scripts' / 'production_workflow.py'
    module_spec = importlib.util.spec_from_file_location('v25_translation_contract_workflow', script)
    workflow = importlib.util.module_from_spec(module_spec); module_spec.loader.exec_module(workflow)
    expected = workflow.source_rows((EVIDENCE.parents[1] / 'PRODUCTION_PLAN.md').read_text())
...
def test_observed_v27_missing_deployment_advisory_is_rejected_by_the_current_contract():
    """v27 omitted the required Japanese rendering of package advisory scans."""
    script = Path(__file__).resolve().parents[1] / 'scripts' / 'production_workflow.py'
    module_spec = importlib.util.spec_from_file_location('v27_deployment_contract_workflow', script)
    workflow = importlib.util.module_from_spec(module_spec); module_spec.loader.exec_module(workflow)
    expected = workflow.source_rows((EVIDENCE.parents[1] / 'PRODUCTION_PLAN.md').read_text())
    raw = (V27 / 'observed-rep01-readiness.json').read_bytes()
    result = workflow.json_schema_check(raw, workflow.delivery_schema(expected))
    assert result['status'] == 'fail'
...
def test_observed_v29_english_and_malformed_audit_terms_are_rejected_by_the_current_contract():
    """v29 passes retained English and malformed audit wording in summaries."""
    script = Path(__file__).resolve().parents[1] / 'scripts' / 'production_workflow.py'
    module_spec = importlib.util.spec_from_file_location('v29_terminology_contract_workflow', script)
    workflow = importlib.util.module_from_spec(module_spec); module_spec.loader.exec_module(workflow)
    expected = workflow.source_rows((EVIDENCE.parents[1] / 'PRODUCTION_PLAN.md').read_text())
    observed = [
        (V29 / 'observed-rep01-readiness.json').read_bytes(),
        (V29 / 'observed-rep02-readiness.json').read_bytes(),
