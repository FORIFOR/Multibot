...
> **Local evaluation:** Ollama trials exposed review, tool-recovery, source-input, artifact-ownership and completion-state bugs. The current checkout passes **105 deterministic tests (1 skipped)**. The legacy 300-run comparison is preserved and stopped because its inputs contain fictional products and synthetic business data; it is not acceptance evidence. The first fixed-series test using the actual repository documents and a local Qwen model was stopped after two runs exposed additional translation drift; the corrected contract rejects those observed fragments before a new series starts. [Fixes](docs/evidence/local-fixes-2026-09-14/README.md) · [Local model results and limitations](docs/evidence/local-qwen35-9b-16k-2026-09-14/README.md).
>
